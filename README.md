...
At Mario's devices Inc. we have developed a set of devices which can monitor weather conditions and air quality. The
information about the devices is collected by a microservice that stores the data into a sql database. A process exports
the data weekly from the sql database as a CSV file and stores it in a landing zone. The CSV contains information about
all the installed devices and is replaced at each execution.

The deployed devices monitor the environmental conditions and send the acquired samples (as soon as possible) to a
message bus. Periodically, a process reads the messages from the bus and writes the received samples to our landing
zone. A new folder is created for each day with the
...
common point like: logging, Session init, config init, input arguments parsing, etc. When you extend SparkApp you have
to define and then specify a derived class of `AbstractContext`, it helps you to handle input arguments, it contains 2
fixed arguments:
...
In the model package there are the beans for the data models, they also contain the constants relative to the name of
the columns, please use these const when you refer to column names to avoid typo and to facilitate rename operations.
...
It reads data from landing zone and loads them in the info delta table. Parameters: no parameters expected here except
the standard ones.
...
```
<year-month-from> : The starting month from which to calculate the report, in the form yyyMM. i.e.: 202001
<year-month-to> : The last month to use to calculate the report. It will be in the form yyyMM. i.e.: 202011
```
...
To launch pipelines execute the usual command spark-submit specifying class and pipeline parameters as said above. The
launch command template is:
...
- run spark-submit with Delta Lake dependencies and the generated jar (below an example of the command for each
  pipeline)
...
- I used to set CI/CD to package a fat jar with all the dependency, deploy it into an artifact repository (like Nexus)
  and copy it into a DFS like S3 o HDFS. I am also used to upgrading the project version in the master branch and
  finally align develop branch with it.
...
There are some stuff we can do to improve this project, some of these improvements are reported inline in the code. Some
other notes are reported below:
...
- It's a good practice to handle range of data as input of the pipeline, it's allow us calculating multiple partition at
  a time and to facilitate possible backfill of data.
- In this project I chose to create Logic classes (instead of objects), then with an internal status with inside session
  and config. At this point we could start a discussion on how to isolate business logic respect config and if we prefer
  OOP respect to functional programming. Just to cite an example of pros and cons:
...
- I had some issues with tests using Spark in different test classes, I try to make the Spark session a Singleton and to
  set sbt to run tests sequentially, but it didn't work. I should spend more time to investigate further to separate
...
- The selected partition strategy optimise reading of data by event date. To optimise reading also by device the
  ...
  of delta lake has been selected, it's disabled when `profile=LOCAL` because the command cannot be used locally.
